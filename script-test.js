        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const interviewForm = document.getElementById('interviewQuestionForm');
            if(interviewForm) {
                interviewForm.addEventListener('submit', function(e) {
                    ...
                    fetch('${pageContext.request.contextPath}/tech/add-interview-question', {
                        method: 'POST',
                        body: formData
                    })
                    .then(response => response.json())
                    .then(data => {
                        if(data.success) {
                            ...
                            // Remove empty state if present
                            const emptyState = document.querySelector('#interviewQuestionList .empty-state');
                            if(emptyState) emptyState.remove();
                            ...
                            newItem.innerHTML = 
                                '<div>' +
                                    ...
                                    '<div class="tags">' +
                                        '<span class="tag tag-type">' + data.question.questionType + '</span>' +
                                        '<span class="tag tag-' + diffClass + '">' + data.question.difficulty + '</span>' +
                                        catTag +
                                    '</div>' +
                                '</div>' +
                                '<form action="${pageContext.request.contextPath}/tech/delete-interview-question/' + data.question.id + '" method="post" onsubmit="return confirm(\'Delete this interview question?\');">' +
                                    '<button class="btn-del" type="submit">' +
                                        '<i class="fas fa-trash-alt"></i> Delete' +
                                    '</button>' +
                                '</form>';
