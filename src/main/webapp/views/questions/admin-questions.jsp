...
<head>
    ...
    <title>Q&A Moderator Panel | JobU - Admin Moderation Center</title>
    ...
    <style>
        ...
        * {
        ...
        .btn-submit {
            background: linear-gradient(105deg, #19A77B, #148F69);
            color: white;
            box-shadow: 0 4px 12px rgba(25, 167, 123,0.3);
        }
...
<jsp:include page="/views/commons/admin_shell_start.jsp">
  <jsp:param name="pageTitle" value="Manage Q&A"/>
  <jsp:param name="pageSubtitle" value="Moderate questions and answers"/>
  <jsp:param name="activeNav" value="qna"/>
</jsp:include>
...
<div class="page-header">
    <div class="container">
        ...
        <h1><i class="fas fa-comments me-2" style="color: var(--accent);"></i>Q&A Moderator Panel</h1>
        <p>Manage questions and provide answers to user inquiries</p>
        <div class="question-count">
            <i class="fas fa-question-circle"></i>
            <span>${fn:length(questions)} Question${fn:length(questions) != 1 ? 's' : ''}</span>
        </div>
    </div>
</div>
    ...
    <c:choose>
        <c:when test="${empty questions}">
            <div class="empty-state">
                ...
                <p>There are no questions to moderate at the moment.</p>
            </div>
        </c:when>
        <c:otherwise>
                ...
                <div class="question-card">
                        ...
                        <div class="question-content">
                            ...
                            <div class="question-meta">
                                <span>
                                    ...
                                    Asked by: <strong>${q.displayName}</strong>
                                </span>
                                <span>
                                    <i class="fas fa-clock"></i>
                                    ${formattedTimestamps[q.id]}
                                </span>
                                <span class="status-badge ${not empty q.answer ? 'status-answered' : 'status-unanswered'}">
                                    <i class="fas fa-${not empty q.answer ? 'check-circle' : 'clock'}"></i>
                                    ${not empty q.answer ? 'Answered' : 'Unanswered'}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <c:if test="${not empty q.answer}">
                        <div class="answer-preview">
                            <strong><i class="fas fa-reply-all"></i> Current Answer</strong>
                            <p>${q.answer.content}</p>
                        </div>
                    </c:if>

                    <!-- Answer Form Section -->
                    <div class="answer-form-section" id="form-${q.id}">
                        <form action="${pageContext.request.contextPath}/qna/admin/qna/answer" method="post">
                            <input type="hidden" name="questionId" value="${q.id}" />
                            <textarea 
                                name="content" 
                                required 
                                placeholder="Write your answer here...">${q.answer.content}</textarea>
                            <div class="mt-3">
                                <button type="submit" class="btn-action btn-submit">
                                    <i class="fas fa-check"></i>
                                    ${not empty q.answer ? 'Update Answer' : 'Post Answer'}
                                </button>
                            </div>
                        </form>
                    </div>
                    ...
                    <div class="question-actions">
                        <button 
                            type="button"
                            class="btn-action btn-answer" 
                            onclick="toggleAnswerForm('${q.id}')">
                            <i class="fas fa-${not empty q.answer ? 'edit' : 'reply'}"></i>
                            ${not empty q.answer ? 'Edit Answer' : 'Answer Question'}
                        </button>

                        <form action="${pageContext.request.contextPath}/qna/admin/qna/delete" 
                              method="post" 
